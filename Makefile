...
FORMULAS=`for i in 1 2 3 4 5; do curl -s "https://api.github.com/orgs/salt-formulas/repos?page=$$i&per_page=100" |grep -E 'clone_url.*salt-formula-' | sed 's/.*salt-formula-\([-_[:alnum:]]*\).git.*/\1/'; done| xargs -n1 | sort -u `;
